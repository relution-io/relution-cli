...
var Server = (function (_super) {
    ...
    Server.prototype.init = function (args) {
        ...
        return rxjs_1.Observable.create(function (observer) {
            if (args.length <= 1) {
                console.log('yoo');
                return _this.showCommands("Choose a " + _this.name.toLocaleUpperCase());
            }
            else if (args.length > 1 && _this.reserved.indexOf(args[1]) === -1) {
                var vars = _this._copy(args);
                vars.splice(0, 1);
                return _this[args[1]](vars);
            }
            else if (args.length > 1 && _this.reserved.indexOf(args[1]) !== -1) {
                //help() quit()
                return _this[args[1]]();
