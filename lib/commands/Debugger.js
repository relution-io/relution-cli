...
var Debugger = (function (_super) {
    ...
    Object.defineProperty(Debugger.prototype, "url", {
        get: function () {
            return this._url;
        },
        set: function (v) {
    ...
    Debugger.prototype.escape = function (s) {
        return s.replace(/"/g, '\\\"');
    };
    Debugger.prototype.open = function () {
        ...
        return this._fileApi.readHjson(path.join(this._deployCommand.projectDir, 'relution.hjson'))
            ...
            var url = choosedServer.serverUrl + "/gofer/security-login?j_username=" + choosedServer.userName + "&j_password=" + choosedServer.password + "&redirect=" + redirect;
            var cmd = 'open --fresh';
            if (os.platform() === 'win32') {
                cmd = 'start';
            }
            return rxjs_1.Observable.create(function (observer) {
                return exec(cmd + " \"" + url + "\"", function (error, stdout, stderr) {
                    if (error) {
