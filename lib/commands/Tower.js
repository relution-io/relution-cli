        ...
        if (this.args[0] === this.name) {
            ...
            if (this.staticCommandRootKeys.indexOf(this.args[0]) !== -1 || this.staticCommandRootKeys.indexOf(this.args[1]) !== -1) {
                ...
                if (subArgs[0] === this.staticCommands[subArgs[0]].name && subArgs.length === 1) {
                    ...
                    return this.staticCommands[subArgs[0]].showCommands().subscribe(function (log) {
                        console.log(log);
                    });
                }
                //['server', 'add']
                ...
                if (this.staticCommands[subArgs[0]][subArgs[1]]) {
                    ...
                    params.splice(0, 2);
                    if (params.length) {
                        return this.staticCommands[subArgs[0]][subArgs[1]](params).subscribe(function (log) {
                            console.log(log);
                        });
                    }
                    return this.staticCommands[subArgs[0]][subArgs[1]]().subscribe(function (log) {
                        console.log('static');
                        console.log(log);
                    });
                }
        ...
        return rxjs_1.Observable.create(function (observer) {
            ...
            _this.flatCommands().forEach(function (commandName) {
                if (commandName !== _this.name && _this.reserved.indexOf(commandName) === -1) {
                    ...
                    content.unshift([chalk.green(_this.name), chalk.cyan(commandName), '', _this.commands[commandName].description]);
                }
            });
            content.unshift(_this._emptyRow);
            if (!asArray) {
