...
var AddConnection = (function () {
    ...
    AddConnection.prototype._addConnectionNamePath = function () {
        ...
        return rxjs_1.Observable.fromPromise(this.connection.inquirer.prompt({
            ...
            validate: function (value) {
                var names = _this.connection.getConnectionNames();
                ...
                if (names.indexOf(value) !== -1) {
                    _this.connection.log.error(new Error("\"" + chalk.magenta(value) + "\" already exist Please choose another one or remove the \"" + chalk.magenta(value + '.hjson') + "\" before."));
                    return false;
                }
                return true;
            }
    ...
    AddConnection.prototype.add = function () {
        ...
        return this._addConnectionNamePath()
            ...
            if (resp.metaModels && resp.metaModels.length) {
                ...
                return _this.connectionModel.metaModel.questions()
                    ...
                    Object.keys(answers).forEach(function (key) {
                        if (key !== _this.connection.i18n.TAKE_ME_OUT) {
                            _this.connectionModel.metaModel.fieldDefinitions[key].defaultValue = answers[key];
                        }
                    });
            ...
            return _this.connection.fileApi.writeFile(template.instance.template, template.instance.name + ".gen.js", _this.connectionHomeFolder);
        })
            .exhaustMap(function () {
            var template = _this._gii.getTemplateByName('connection');
            template.instance.name = _this.connectionName;
            template.instance.path = path.dirname(_this.connectionModel.name);
            return _this.connection.fileApi.writeFile(template.instance.template, template.instance.name + ".js", _this._rootFolder)
                .do(function (file) {
                if (fileWritten) {
