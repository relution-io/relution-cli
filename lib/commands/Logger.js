...
var Logger = (function (_super) {
    ...
    Logger.prototype._beautifyLogMessage = function (log) {
        ...
        var content = [[this.color.underline[bgColor](this.color.white(levelName)), log.message, this._getHumanDate(log.date), log.id]];
                ...
                if (messages.length) {
                    messages.map(function (log) {
                        ...
                        if (os.platform() === 'win32' || _this._state === 'args') {
                            console.log(_this._beautifyLogMessage(log));
                        }
                        else {
                            _this.termLog.log(_this._beautifyLogMessage(log), { height: 30 });
                        }
                    });
                }
