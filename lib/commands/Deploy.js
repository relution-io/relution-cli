...
var Deploy = (function (_super) {
    ...
    Deploy.prototype.getServerPrompt = function () {
        ...
        var indexDefault = lodash_1.findIndex(this.userRc.server, { default: true });
        if (indexDefault > -1) {
    ...
    Deploy.prototype.upload = function (archiveresp, env) {
        ...
        return rxjs_1.Observable.fromPromise(Relution.web.ajax({
            ...
            _this.log.info(_this.i18n.DEPLOY_SUCCESS);
            // be nice and output URL of application
            var url = Relution.web.resolveApp(_this._relutionHjson);
            if (url) {
                _this.log.info(_this.i18n.DEPLOY_APPURL, url);
            }
            return result;
        }, function (error) {
            _this.log.error(new Error(_this.i18n.DEPLOY_FAILED));
            throw error;
        }));
    };
    /**
    ...
    Deploy.prototype.publish = function () {
        ...
        return this._parent.staticCommands.env.envCollection.getEnvironments()
            ...
            _this.log.info(chalk.green("Login as " + (userResp.givenName ? userResp.givenName + ' ' + userResp.surname : userResp.name) + " succeeded. " + figures.tick));
