...
var Environment = (function (_super) {
    ...
    Environment.prototype.createEnvironment = function (name) {
        ...
        return rxjs_1.Observable.create(function (observer) {
            ...
            _this.fsApi.writeHjson(template.instance.render(name), name).subscribe(function (pipe) {
                observer.next(pipe);
            }, function (e) { console.error(e); observer.error(e); }, function () {
    ...
    Environment.prototype.update = function (name) {
        ...
        var names = [];
        return rxjs_1.Observable.create(function (observer) {
            _this.chooseEnv.choose().subscribe(function (answers) {
    ...
    Environment.prototype.add = function (name) {
        ...
        if (lodash_1.isArray(name)) {
            ...
            if (unique) {
                return rxjs_1.Observable.create(function (observer) {
                    observer.next(chalk.red("\n Name " + envName_1 + " already exist please choose another one"));
                    observer.complete();
                });
            }
            if (pass) {
                return rxjs_1.Observable.create(function (observer) {
                    _this.createEnvironment(envName_1).subscribe(function () { observer.next(chalk.green(envName_1 + ".hjson are written")); }, function (e) { observer.error(e); }, function () { observer.complete(); });
                });
            }
            return rxjs_1.Observable.create(function (observer) {
                observer.next(chalk.red("\n Name " + envName_1 + " has wrong character allowed only [a-z A-Z]"));
                observer.complete();
            });
        }
    };
