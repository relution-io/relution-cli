...
export class EnvCollection {
  ...
  public copyByName(name: string, to: string): Observable<any> {
    if (!this.validate(name) || !this.validate(to)) {
      return Observable.create((observer: any) => {
        ...
        Only Allowed ${Validator.stringPattern}`));
    ...
    return Observable.create((observer: any) => {
      ...
      this.fsApi.writeHjson(hjson.stringify(newData, this.fsApi.hjsonOptions), to)
        .subscribe({
          complete: () => {
            this.getEnvironments().subscribe({
              next: () => {
   ...
   */
  public updateModelByName(name: string, attributes: Array<{
    key: string,
    value: any
  }>) {
    return Observable.create((observer: any) => {
  ...
  public bulkUpdate(env: Array<string>, store: Array<{
    key: string,
    value: any
  }>): Observable<any> {
    if (!env.length) {
