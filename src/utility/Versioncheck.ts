...
export class NpmVersionCheck {
  ...
  private static _versionCheck() {
    if (!NpmVersionCheck._pkg) {
      return emoji.emojify(`:interrobang: ${Translation.CLI_VERSION_CHECK_FAILED}`);
    }
    return NpmVersionCheck._pkg.version[0] !== pkg.version ?
      emoji.emojify(`:warning: ${chalk.yellow(Translation.CLI_OUTOFDATE(NpmVersionCheck._pkg.version))}`) :
